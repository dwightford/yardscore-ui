...
/**
 * EmailCodeAuthForm — shared 6-digit email-code auth surface.
 *
 * Rendered by both /login and /register. The underlying flow is identical;
 * `mode` only tunes copy and the counterpart-link destination.
 *
 * Canon: yardscore-auth-first-entry-and-claiming-v1. Keep the 6-digit
 * email-code auth — do not replace with passwords.
 */
...
export default function EmailCodeAuthForm({
  mode,
}: {
  ...
  function counterpartHref(): string {
    const target = mode === "signin" ? "/register" : "/login";
    const q = new URLSearchParams();
    if (mode === "register") q.set("mode", "signin");
    if (nextRaw !== "/dashboard") q.set("next", nextRaw);
      ...
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="mx-auto w-14 h-14 rounded-2xl bg-lime-300/10 border border-lime-300/20 flex items-center justify-center mb-4">
            <svg
              viewBox="0 0 24 24"
              className="w-7 h-7 text-lime-300"
              fill="none"
              stroke="currentColor"
              strokeWidth="1.5"
            >
          ...
          {address && intent === "claim" && (
            <p className="mt-3 rounded-lg bg-lime-300/5 border border-lime-300/20 px-3 py-2 text-xs text-lime-200">
              You&apos;ll claim <span className="font-medium">{address}</span> after
              you sign in.
            </p>
          )}
          {address && intent === "request_access" && (
            <p className="mt-3 rounded-lg bg-white/5 border border-white/10 px-3 py-2 text-xs text-zinc-300">
              You&apos;ll request access to{" "}
              <span className="font-medium">{address}</span> after you sign in.
            </p>
          )}
        </div>
        ...
        <div className="mt-8 text-center space-y-2">
          <a href={counterpartHref()} className="block text-xs text-zinc-400 hover:text-zinc-200">
            {counterpartLabel}
          </a>
          <a href="/" className="block text-xs text-zinc-500 hover:text-zinc-300">
            ← Back to YardScore
          </a>
          <p className="text-[10px] text-zinc-600 pt-2">Build {BUILD_SHA}</p>
        </div>
      </div>
    </div>
  );
