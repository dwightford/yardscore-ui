 ...
 * BottomTabs
 *
 * Mobile: Home / Observe / Map / Profile
 * Desktop: Home / Map / Profile (no Observe — mobile observes, web interprets)
 */
...
function useIsMobile() {
  ...
  useEffect(() => {
    // Touch support + narrow viewport = mobile
    const check = () =>
      setMobile("ontouchstart" in window || navigator.maxTouchPoints > 0);
    check();
    // No need to re-check — device type doesn't change mid-session
  }, []);
  return mobile;
}
